...
tf.flags.DEFINE_integer("num_filters", 160, "Number of filters per filter size (default: 128)")
tf.flags.DEFINE_float("dropout_keep_prob", 0.65, "Dropout keep probability (default: 0.5)")
tf.flags.DEFINE_float("l2_reg_lambda", 7.0, "L2 regularization lambda (default: 0.0)")

# Training parameters
tf.flags.DEFINE_integer("batch_size", 176, "Batch Size (default: 64)")
...
tf.flags.DEFINE_float("eps",1.26,"defaut is 2")
...
def data_scale():
    ...
    #training data scale is df/math.sqrt(FLAGS.eps*2)
    #where dF is the max filter size
    df=float(FLAGS.filter_sizes.split(",")[-1])
    print(df)
    scale=df/math.sqrt(FLAGS.eps*2)
    return scale

def gradient_scale(data_size):
